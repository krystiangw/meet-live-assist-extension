...
import { fillPanel, PANEL_URL, OUT, RELEASE_REVIEW, PLANNING } from './panel.mjs';
...
for (const [file, script] of [['panel-full.png', RELEASE_REVIEW], ['panel-planning.png', PLANNING]]) {
  const page = await browser.newPage({ viewport: { width: 420, height: 1000 }, deviceScaleFactor: 2 });
  await page.goto(PANEL_URL);
  await fillPanel(page, script);
